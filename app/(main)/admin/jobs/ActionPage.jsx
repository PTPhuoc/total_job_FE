...
export default function ActionPage({ listJob }) {
  const windowWarning = useSelector((state) => state.windowWarning);
  const dispatch = useDispatch();
  const route = useRouter();
  const [jobs, setJobs] = useState(listJob.listJob);
  const [searchValue, setSearchValue] = useState({ job: "" });
  const [changePage, setChangePage] = useState({
    page: listJob.page,
    total: listJob.total,
    pageSize: listJob.limit,
    totalPage: listJob.totalPages,
  });
  ...
  useEffect(() => {
    const handleWindow = async () => {
      if (windowWarning.handle === "accept") {
        if (windowWarning.for === "DeleteJobByAdmin") {
          await deleteJob(windowWarning.value);
        }
    ...
    <div className="flex min-w-0 flex-col gap-1 w-full h-full">
      <div className="flex gap-5 items-center h-[100px] px-5 bg-white">
        ...
        <ButtonDefault
          ...
          textButton={"Tìm kiếm"}
          handleApi={() => {
            return searchJob(changePage.page);
          }}
        ></ButtonDefault>
      </div>
      {jobs && jobs.length > 0 ? (
          ...
          {jobs.map((item, index) => (
            ...
            >
              <div className="flex flex-1 min-w-0 flex-col gap-2">
                ...
                <p className="font-bold text-zinc-500">{item.company.name}</p>
              </div>
