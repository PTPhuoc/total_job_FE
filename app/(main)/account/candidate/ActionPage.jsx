...
export default function ActionPage({
  ...
  const [school, setSchool] = useState(schoolCatalog);
  const [skill, setSkill] = useState(skillCatalog);
  const [inputValue, setInputValue] = useState(() => {
    if (account && account.candidate) {
      return {
        name: account.candidate.name ? account.candidate.name : "",
        birthday: account.candidate.birthday ? account.candidate.birthday : "",
        decryption: account.candidate.decryption
          ? account.candidate.decryption
          : "",
        address: account.candidate.address ? account.candidate.address : "",
        exp: account.candidate.exp ? account.candidate.exp : "",
        education: account.candidate.education
          ? account.candidate.education
          : "",
        skills: account.candidate.skills ? account.candidate.skills : "",
        cv_url: account.candidate.cv_url ? account.candidate.cv_url : "",
        image: account.candidate.image ? account.candidate.image : "",
      };
    }
    return {
      name: "",
    ...
    <>
      ...
      <div className="flex-2 flex flex-col p-5 gap-5 bg-white rounded-2xl shadow-basic overflow-auto no-scroll">
        ...
        <div className="z-[5] flex flex-col p-5 border-2 border-zinc-200 rounded-xl">
          ...
          <InputListSearch
            className="outline-none border-b-2 border-zinc-200 duration-200 ease-in focus:border-[#01215C]"
            inputValue={inputValue.education}
            inputChange={(value) =>
              setInputValue({ ...inputValue, education: value })
            }
            listSearch={school}
            ...
            showlist={(item, index) => {
              return (
                <div
                  ...
                  onClick={() => {
                    setInputValue({ ...inputValue, education: item.name });
                  }}
                >
                  {item.name}
                </div>
